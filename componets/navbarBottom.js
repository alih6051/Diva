const navbarBottom = () => {
    return `<nav class="navbar navbar-expand-md navbar-light bg-white">
    <div class="container">
      <div class="collapse navbar-collapse" id="navbarCollapse">
        <ul class="navbar-nav me-auto mb-2 mb-md-0">
        <li class="dropdown mega-dropdown">
        <a href="#" class="navbarBottomMenu">Makeup <span class="caret"></span></a>				
        <ul class="dropdown-menu mega-dropdown-menu">
        <div class="row">
        <li class="col-sm-2">
            <ul>
            <li class="dropdown-header">What's New</li>
            <li><a href="#">Recently Added</a></li>
             
            <li class="dropdown-header">Women</li>
            <li><a href="#">Indianwear</a></li>
            <li><a href="#">Westernwear</a></li>
            <li><a href="#">Footwear</a></li>
            <li><a href="#">Bags</a></li>
            <li><a href="#">Lingerie</a></li>
            <li><a href="#">Sportswear</a></li>
            <li><a href="#">Sleep & Lounge</a></li>
            <li><a href="#">Jewellery</a></li>
            <li><a href="#">Watches</a></li>
            <li><a href="#">Accessories</a></li>
          </ul>
            </li>
            <li class="col-sm-2">
              <ul>
                <li class="dropdown-header">Men</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
            </li>
            <li class="col-sm-2">
              <ul>
              <li class="dropdown-header">Shop by Diva</li>
              <li><a href="#">Sportswear</a></li>
              <li><a href="#">Sleep & Lounge</a></li>
              <li><a href="#">Jewellery</a></li>
              <li><a href="#">Watches</a></li>
              <li><a href="#">Accessories</a></li>						
              </ul>
            </li>
            <li class="col-sm-2">
              <ul>
                <li class="dropdown-header">Top Brands</li>
                <li><a href="#">Rebook</a></li>
                <li><a href="#">Puma</a></li>
                <li><a href="#">Addidas</a></li>
                <li><a href="#">Campus</a></li>
                <li><a href="#">Woodland</a></li>		                        
              </ul>
            </li>
        </div>
        </ul>				
      </li>
      <li class="dropdown mega-dropdown">
      <a href="./Products_page/Hair_Product/skin.html" class="navbarBottomMenu">Skin <span class="caret"></span></a>				
      <ul class="dropdown-menu mega-dropdown-menu">
      <div class="row">
      <li class="col-sm-2">
      <ul>
        <li class="dropdown-header">Face</li>
        <li><a href="#">Face Primer</a></li>
        <li><a href="#">Concealer</a></li>
        <li><a href="#">Foundation</a></li>
        <li><a href="#">Compact</a></li>
        <li><a href="#">Contour</a></li>
        <li><a href="#">Loose Powder</a></li>
        <li><a href="#">Tinted Moisturizer</a></li>
        <li><a href="#">Blush</a></li>
        <li><a href="#">Bronzer</a></li>
        <li><a href="#">BB & CC cream</a></li>
        <li><a href="#">Highlighters</a></li>
        <li><a href="#">Setting Spray</a></li>
        <li><a href="#">Makeup Remover</a></li>
        <li><a href="#">Sindoor</a></li>
      </ul>
      </li>
      <li class="col-sm-2">
      <ul>
        <li class="dropdown-header">Eye</li>
        <li><a href="#">Kajal</a></li>
        <li><a href="#">Eyeliner</a></li>
        <li><a href="#">Mascara</a></li>
        <li><a href="#">Eye Shadow</a></li>
        <li><a href="#">Eye Brow Enhancers</a></li>
        <li><a href="#">Eye Primer</a></li>
        <li><a href="#">False Eyelashes</a></li>
        <li><a href="#">Eye Makeup Remover</a></li>
        <li><a href="#">Under Eye Concealer</a></li>
        <li><a href="#">Contact Lenses</a></li>
      </ul>
      </li>
      <li class="col-sm-2">
      <ul>
      <li class="dropdown-header">What's New</li>
      <li><a href="#">Recently Added</a></li>
      <li class="dropdown-header">Women</li>
      <li><a href="#">Indianwear</a></li>
      <li><a href="#">Westernwear</a></li>
      <li><a href="#">Footwear</a></li>
      <li><a href="#">Bags</a></li>
      <li><a href="#">Lingerie</a></li>
      <li><a href="#">Sportswear</a></li>
      <li><a href="#">Sleep & Lounge</a></li>
      <li><a href="#">Jewellery</a></li>
      <li><a href="#">Watches</a></li>
      <li><a href="#">Accessories</a></li>
    </ul>
      </li>
      <li class="col-sm-2">
        <ul>
          <li class="dropdown-header">Men</li>
          <li><a href="#">Indianwear</a></li>
          <li><a href="#">Westernwear</a></li>
          <li><a href="#">Footwear</a></li>
          <li><a href="#">Bags</a></li>
          <li><a href="#">Lingerie</a></li>
          <li><a href="#">Sportswear</a></li>
          <li><a href="#">Sleep & Lounge</a></li>
          <li><a href="#">Jewellery</a></li>
          <li><a href="#">Watches</a></li>
          <li><a href="#">Accessories</a></li>
        </ul>
      </li>
      <li class="col-sm-2">
        <ul>
        <li class="dropdown-header">Shop by Diva</li>
        <li><a href="#">Sportswear</a></li>
        <li><a href="#">Sleep & Lounge</a></li>
        <li><a href="#">Jewellery</a></li>
        <li><a href="#">Watches</a></li>
        <li><a href="#">Accessories</a></li>						
        </ul>
      </li>
      <li class="col-sm-2">
        <ul>
          <li class="dropdown-header">Top Brands</li>
          <li><a href="#">Rebook</a></li>
          <li><a href="#">Puma</a></li>
          <li><a href="#">Addidas</a></li>
          <li><a href="#">Campus</a></li>
          <li><a href="#">Woodland</a></li>		                        
        </ul>
      </li>
        <div>
      </ul>				
    </li>
    <li class="dropdown mega-dropdown">
        <a href="./Products_page/Hair_Product/hair.html" class="navbarBottomMenu">Hair <span class="caret"></span></a>				
        <ul class="dropdown-menu mega-dropdown-menu">
        <div class="row">
        <li class="col-sm-2">
        <ul>
        <li class="dropdown-header">What's New</li>
        <li><a href="#">Recently Added</a></li>
         
        <li class="dropdown-header">Women</li>
        <li><a href="#">Indianwear</a></li>
        <li><a href="#">Westernwear</a></li>
        <li><a href="#">Footwear</a></li>
        <li><a href="#">Bags</a></li>
        <li><a href="#">Lingerie</a></li>
        <li><a href="#">Sportswear</a></li>
        <li><a href="#">Sleep & Lounge</a></li>
        <li><a href="#">Jewellery</a></li>
        <li><a href="#">Watches</a></li>
        <li><a href="#">Accessories</a></li>
      </ul>
        </li>
        <li class="col-sm-2">
          <ul>
            <li class="dropdown-header">Men</li>
            <li><a href="#">Indianwear</a></li>
            <li><a href="#">Westernwear</a></li>
            <li><a href="#">Footwear</a></li>
            <li><a href="#">Bags</a></li>
            <li><a href="#">Lingerie</a></li>
            <li><a href="#">Sportswear</a></li>
            <li><a href="#">Sleep & Lounge</a></li>
            <li><a href="#">Jewellery</a></li>
            <li><a href="#">Watches</a></li>
            <li><a href="#">Accessories</a></li>
          </ul>
        </li>
        <li class="col-sm-2">
          <ul>
          <li class="dropdown-header">Shop by Diva</li>
          <li><a href="#">Sportswear</a></li>
          <li><a href="#">Sleep & Lounge</a></li>
          <li><a href="#">Jewellery</a></li>
          <li><a href="#">Watches</a></li>
          <li><a href="#">Accessories</a></li>						
          </ul>
        </li>
        <li class="col-sm-2">
          <ul>
            <li class="dropdown-header">Top Brands</li>
            <li><a href="#">Rebook</a></li>
            <li><a href="#">Puma</a></li>
            <li><a href="#">Addidas</a></li>
            <li><a href="#">Campus</a></li>
            <li><a href="#">Woodland</a></li>		                        
          </ul>
        </li>
        <div>
        </ul>				
        </li>
          <li class="dropdown mega-dropdown">
          <a href="#" class="navbarBottomMenu">Appliances <span class="caret"></span></a>				
          <ul class="dropdown-menu mega-dropdown-menu">
          <div class="row">
          <li class="col-sm-2">
          <ul>
          <li class="dropdown-header">What's New</li>
          <li><a href="#">Recently Added</a></li>
           
          <li class="dropdown-header">Women</li>
          <li><a href="#">Indianwear</a></li>
          <li><a href="#">Westernwear</a></li>
          <li><a href="#">Footwear</a></li>
          <li><a href="#">Bags</a></li>
          <li><a href="#">Lingerie</a></li>
          <li><a href="#">Sportswear</a></li>
          <li><a href="#">Sleep & Lounge</a></li>
          <li><a href="#">Jewellery</a></li>
          <li><a href="#">Watches</a></li>
          <li><a href="#">Accessories</a></li>
        </ul>
          </li>
          <li class="col-sm-2">
            <ul>
              <li class="dropdown-header">Men</li>
              <li><a href="#">Indianwear</a></li>
              <li><a href="#">Westernwear</a></li>
              <li><a href="#">Footwear</a></li>
              <li><a href="#">Bags</a></li>
              <li><a href="#">Lingerie</a></li>
              <li><a href="#">Sportswear</a></li>
              <li><a href="#">Sleep & Lounge</a></li>
              <li><a href="#">Jewellery</a></li>
              <li><a href="#">Watches</a></li>
              <li><a href="#">Accessories</a></li>
            </ul>
          </li>
          <li class="col-sm-2">
            <ul>
            <li class="dropdown-header">Shop by Diva</li>
            <li><a href="#">Sportswear</a></li>
            <li><a href="#">Sleep & Lounge</a></li>
            <li><a href="#">Jewellery</a></li>
            <li><a href="#">Watches</a></li>
            <li><a href="#">Accessories</a></li>						
            </ul>
          </li>
          <li class="col-sm-2">
            <ul>
              <li class="dropdown-header">Top Brands</li>
              <li><a href="#">Rebook</a></li>
              <li><a href="#">Puma</a></li>
              <li><a href="#">Addidas</a></li>
              <li><a href="#">Campus</a></li>
              <li><a href="#">Woodland</a></li>		                        
            </ul>
          </li>
            <div>
          </ul>				
        </li>
        <li class="dropdown mega-dropdown">
            <a href="#" class="navbarBottomMenu">Bath & Beauty <span class="caret"></span></a>				
            <ul class="dropdown-menu mega-dropdown-menu">
                <div class="row">
                <li class="col-sm-2">
                <ul>
                <li class="dropdown-header">What's New</li>
                <li><a href="#">Recently Added</a></li>
                 
                <li class="dropdown-header">Women</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Men</li>
                    <li><a href="#">Indianwear</a></li>
                    <li><a href="#">Westernwear</a></li>
                    <li><a href="#">Footwear</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Lingerie</a></li>
                    <li><a href="#">Sportswear</a></li>
                    <li><a href="#">Sleep & Lounge</a></li>
                    <li><a href="#">Jewellery</a></li>
                    <li><a href="#">Watches</a></li>
                    <li><a href="#">Accessories</a></li>
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                  <li class="dropdown-header">Shop by Diva</li>
                  <li><a href="#">Sportswear</a></li>
                  <li><a href="#">Sleep & Lounge</a></li>
                  <li><a href="#">Jewellery</a></li>
                  <li><a href="#">Watches</a></li>
                  <li><a href="#">Accessories</a></li>						
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Top Brands</li>
                    <li><a href="#">Rebook</a></li>
                    <li><a href="#">Puma</a></li>
                    <li><a href="#">Addidas</a></li>
                    <li><a href="#">Campus</a></li>
                    <li><a href="#">Woodland</a></li>		                        
                  </ul>
                </li>
                <div>
                </ul>				
                </li>
                <li class="dropdown mega-dropdown">
                <a href="#" class="navbarBottomMenu">Mom & Baby <span class="caret"></span></a>				
                <ul class="dropdown-menu mega-dropdown-menu">
                <div class="row">
                <li class="col-sm-2">
                <ul>
                <li class="dropdown-header">What's New</li>
                <li><a href="#">Recently Added</a></li>
                 
                <li class="dropdown-header">Women</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Men</li>
                    <li><a href="#">Indianwear</a></li>
                    <li><a href="#">Westernwear</a></li>
                    <li><a href="#">Footwear</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Lingerie</a></li>
                    <li><a href="#">Sportswear</a></li>
                    <li><a href="#">Sleep & Lounge</a></li>
                    <li><a href="#">Jewellery</a></li>
                    <li><a href="#">Watches</a></li>
                    <li><a href="#">Accessories</a></li>
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                  <li class="dropdown-header">Shop by Diva</li>
                  <li><a href="#">Sportswear</a></li>
                  <li><a href="#">Sleep & Lounge</a></li>
                  <li><a href="#">Jewellery</a></li>
                  <li><a href="#">Watches</a></li>
                  <li><a href="#">Accessories</a></li>						
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Top Brands</li>
                    <li><a href="#">Rebook</a></li>
                    <li><a href="#">Puma</a></li>
                    <li><a href="#">Addidas</a></li>
                    <li><a href="#">Campus</a></li>
                    <li><a href="#">Woodland</a></li>		                        
                  </ul>
                </li>
                <div>
                </ul>				
                </li>
                <li class="dropdown mega-dropdown">
                <a href="#" class="navbarBottomMenu">Natural <span class="caret"></span></a>				
                <ul class="dropdown-menu mega-dropdown-menu">
                <div class="row">
                <li class="col-sm-2">
                <ul>
                <li class="dropdown-header">What's New</li>
                <li><a href="#">Recently Added</a></li>
                 
                <li class="dropdown-header">Women</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Men</li>
                    <li><a href="#">Indianwear</a></li>
                    <li><a href="#">Westernwear</a></li>
                    <li><a href="#">Footwear</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Lingerie</a></li>
                    <li><a href="#">Sportswear</a></li>
                    <li><a href="#">Sleep & Lounge</a></li>
                    <li><a href="#">Jewellery</a></li>
                    <li><a href="#">Watches</a></li>
                    <li><a href="#">Accessories</a></li>
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                  <li class="dropdown-header">Shop by Diva</li>
                  <li><a href="#">Sportswear</a></li>
                  <li><a href="#">Sleep & Lounge</a></li>
                  <li><a href="#">Jewellery</a></li>
                  <li><a href="#">Watches</a></li>
                  <li><a href="#">Accessories</a></li>						
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Top Brands</li>
                    <li><a href="#">Rebook</a></li>
                    <li><a href="#">Puma</a></li>
                    <li><a href="#">Addidas</a></li>
                    <li><a href="#">Campus</a></li>
                    <li><a href="#">Woodland</a></li>		                        
                  </ul>
                </li>
                <div>
                </ul>				
                </li>
                <li class="dropdown mega-dropdown">
                <a href="#" class="navbarBottomMenu">Health & Wellness<span class="caret"></span></a>				
                <ul class="dropdown-menu mega-dropdown-menu">
                <div class="row">
                <li class="col-sm-2">
                <ul>
                <li class="dropdown-header">What's New</li>
                <li><a href="#">Recently Added</a></li>
                 
                <li class="dropdown-header">Women</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Men</li>
                    <li><a href="#">Indianwear</a></li>
                    <li><a href="#">Westernwear</a></li>
                    <li><a href="#">Footwear</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Lingerie</a></li>
                    <li><a href="#">Sportswear</a></li>
                    <li><a href="#">Sleep & Lounge</a></li>
                    <li><a href="#">Jewellery</a></li>
                    <li><a href="#">Watches</a></li>
                    <li><a href="#">Accessories</a></li>
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                  <li class="dropdown-header">Shop by Diva</li>
                  <li><a href="#">Sportswear</a></li>
                  <li><a href="#">Sleep & Lounge</a></li>
                  <li><a href="#">Jewellery</a></li>
                  <li><a href="#">Watches</a></li>
                  <li><a href="#">Accessories</a></li>						
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Top Brands</li>
                    <li><a href="#">Rebook</a></li>
                    <li><a href="#">Puma</a></li>
                    <li><a href="#">Addidas</a></li>
                    <li><a href="#">Campus</a></li>
                    <li><a href="#">Woodland</a></li>		                        
                  </ul>
                </li>
                <div>
                </ul>				
                </li>
                <li class="dropdown mega-dropdown">
                <a href="#" class="navbarBottomMenu">Men <span class="caret"></span></a>				
                <ul class="dropdown-menu mega-dropdown-menu">
                <div class="row">
                <li class="col-sm-2">
                <ul>
                <li class="dropdown-header">What's New</li>
                <li><a href="#">Recently Added</a></li>
                 
                <li class="dropdown-header">Women</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Men</li>
                    <li><a href="#">Indianwear</a></li>
                    <li><a href="#">Westernwear</a></li>
                    <li><a href="#">Footwear</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Lingerie</a></li>
                    <li><a href="#">Sportswear</a></li>
                    <li><a href="#">Sleep & Lounge</a></li>
                    <li><a href="#">Jewellery</a></li>
                    <li><a href="#">Watches</a></li>
                    <li><a href="#">Accessories</a></li>
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                  <li class="dropdown-header">Shop by Diva</li>
                  <li><a href="#">Sportswear</a></li>
                  <li><a href="#">Sleep & Lounge</a></li>
                  <li><a href="#">Jewellery</a></li>
                  <li><a href="#">Watches</a></li>
                  <li><a href="#">Accessories</a></li>						
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Top Brands</li>
                    <li><a href="#">Rebook</a></li>
                    <li><a href="#">Puma</a></li>
                    <li><a href="#">Addidas</a></li>
                    <li><a href="#">Campus</a></li>
                    <li><a href="#">Woodland</a></li>		                        
                  </ul>
                </li>
                <div>
                </ul>				
                </li>
                <li class="dropdown mega-dropdown">
                <a href="#" class="navbarBottomMenu">Fragrance<span class="caret"></span></a>				
                <ul class="dropdown-menu mega-dropdown-menu">
                <div class="row">
                <li class="col-sm-2">
                <ul>
                <li class="dropdown-header">What's New</li>
                <li><a href="#">Recently Added</a></li>
                 
                <li class="dropdown-header">Women</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Men</li>
                    <li><a href="#">Indianwear</a></li>
                    <li><a href="#">Westernwear</a></li>
                    <li><a href="#">Footwear</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Lingerie</a></li>
                    <li><a href="#">Sportswear</a></li>
                    <li><a href="#">Sleep & Lounge</a></li>
                    <li><a href="#">Jewellery</a></li>
                    <li><a href="#">Watches</a></li>
                    <li><a href="#">Accessories</a></li>
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                  <li class="dropdown-header">Shop by Diva</li>
                  <li><a href="#">Sportswear</a></li>
                  <li><a href="#">Sleep & Lounge</a></li>
                  <li><a href="#">Jewellery</a></li>
                  <li><a href="#">Watches</a></li>
                  <li><a href="#">Accessories</a></li>						
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Top Brands</li>
                    <li><a href="#">Rebook</a></li>
                    <li><a href="#">Puma</a></li>
                    <li><a href="#">Addidas</a></li>
                    <li><a href="#">Campus</a></li>
                    <li><a href="#">Woodland</a></li>		                        
                  </ul>
                </li>
                <div>
                </ul>				
                </li>
                <li class="dropdown mega-dropdown">
                <a href="#" class="navbarBottomMenu">PopUps <span class="caret"></span></a>				
                <ul class="dropdown-menu mega-dropdown-menu">
                <div class="row">
                <li class="col-sm-2">
                <ul>
                <li class="dropdown-header">What's New</li>
                <li><a href="#">Recently Added</a></li>
                 
                <li class="dropdown-header">Women</li>
                <li><a href="#">Indianwear</a></li>
                <li><a href="#">Westernwear</a></li>
                <li><a href="#">Footwear</a></li>
                <li><a href="#">Bags</a></li>
                <li><a href="#">Lingerie</a></li>
                <li><a href="#">Sportswear</a></li>
                <li><a href="#">Sleep & Lounge</a></li>
                <li><a href="#">Jewellery</a></li>
                <li><a href="#">Watches</a></li>
                <li><a href="#">Accessories</a></li>
              </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Men</li>
                    <li><a href="#">Indianwear</a></li>
                    <li><a href="#">Westernwear</a></li>
                    <li><a href="#">Footwear</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Lingerie</a></li>
                    <li><a href="#">Sportswear</a></li>
                    <li><a href="#">Sleep & Lounge</a></li>
                    <li><a href="#">Jewellery</a></li>
                    <li><a href="#">Watches</a></li>
                    <li><a href="#">Accessories</a></li>
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                  <li class="dropdown-header">Shop by Diva</li>
                  <li><a href="#">Sportswear</a></li>
                  <li><a href="#">Sleep & Lounge</a></li>
                  <li><a href="#">Jewellery</a></li>
                  <li><a href="#">Watches</a></li>
                  <li><a href="#">Accessories</a></li>						
                  </ul>
                </li>
                <li class="col-sm-2">
                  <ul>
                    <li class="dropdown-header">Top Brands</li>
                    <li><a href="#">Rebook</a></li>
                    <li><a href="#">Puma</a></li>
                    <li><a href="#">Addidas</a></li>
                    <li><a href="#">Campus</a></li>
                    <li><a href="#">Woodland</a></li>		                        
                  </ul>
                </li>
                <div>
                </ul>				
                </li>
                <li class="dropdown mega-dropdown">
                <img src="./resouces/images/offer.png" alt="" style="height: 26px;margin: 0;">
                </li>
        </ul>
      </div>
    </div>
  </nav>`
}

export default navbarBottom;